...
class Session {
	...
	static Future<int> fetchLoginUsers() async {
		final list = DbCenter.db.query(DbCenter.tbUserModel,
				Conditions().add(Condition(ConditionType.DefinedNotNull)..key = Keys.lastLoginDate));
	...
	static bool isLogin(int userId){
	...
	static Future<UserModel?> login$newProfileData(Map<String, dynamic> json) async {
		final userId = Converter.dynamicToIntNull(json[Keys.userId]);
		final String? userName = json[Keys.userName];
		...
		/// insert to db
		final updateDb = await DbCenter.db.insertOrUpdate(DbCenter.tbUserModel, newUser.toMap(),
	...
	static UserModel? getExistLoginUserById(int userId){
	...
	static Future<UserModel?> fetchUserById(int userId) async {
		final cas = DbCenter.db.query(DbCenter.tbUserModel,
	...
	static Future<UserModel?> pickUserById(int userId) async{
	...
	static Future<bool> sinkUserInfo(UserModel user) async {
		...
		final res = await DbCenter.db.update(DbCenter.tbUserModel, user.toMap(),
	...
	static Future<bool> logoff(int userId) async{
		...
		val[Keys.lastLoginDate] = null;

		await DbCenter.db.update(DbCenter.tbUserModel, val,
				Conditions().add(Condition()..key = Keys.userId..value = userId));
	...
	static Future<bool> deleteUserInfo(int userId) async{
		final res = await DbCenter.db.delete(DbCenter.tbUserModel,
				Conditions().add(Condition()..key = Keys.userId..value = userId));
	...
	static String getSexEquivalent(int? sex){
		if(sex == null) {
		  return LocaleCenter.appLocalize.translate('unknown')!;
		}
		...
		switch(sex){
			case 0:
				return LocaleCenter.appLocalize.translate('unknown')!;
			case 1:
				return LocaleCenter.appLocalize.translate('man')!;
			case 2:
				return LocaleCenter.appLocalize.translate('woman')!;
			case 5:
				return LocaleCenter.appLocalize.translate('bisexual')!;
		}
